...
fn parse_input(input: &str) -> (Warehouse, (usize, usize)) {
    ...
    let mut out_matrix =
        nalgebra::DMatrix::from_element(warehouse_init.len(), warehouse_init[0].len(), Item::Empty);
...
fn parse_input_two(
    ...
    nalgebra::Matrix<
        ...
        nalgebra::Dyn,
        nalgebra::Dyn,
        nalgebra::VecStorage<ItemTwo, nalgebra::Dyn, nalgebra::Dyn>,
    >,
    Vec<Move>,
    (usize, usize),
) {
    let parts: Vec<&str> = input.split("\n\n").collect();
    let warehouse_init: Vec<&str> = parts[0].split("\n").collect();
    let mut out_matrix = nalgebra::DMatrix::from_element(
        warehouse_init.len(),
        warehouse_init[0].len() * 2,
        ItemTwo::Empty,
    );
    let mut start_position = (0, 0);
    for (y, line) in warehouse_init.into_iter().enumerate() {
...
fn calc_gps_score(
    contents: nalgebra::Matrix<
        Item,
        nalgebra::Dyn,
        nalgebra::Dyn,
        nalgebra::VecStorage<Item, nalgebra::Dyn, nalgebra::Dyn>,
    >,
) -> u32 {
    let mut sum_score = 0;
...
fn movement_target(position: (usize, usize), dir: &Move) -> (usize, usize) {
    match r#dir {
        Move::Down => (position.0 + 1, position.1),
        Move::Left => (position.0, position.1 - 1),
        Move::Right => (position.0, position.1 + 1),
        Move::Up => (position.0 - 1, position.1),
    }
}
...
fn try_move(
    start: (usize, usize),
    dir: &Move,
    contents: &mut nalgebra::DMatrix<Item>,
) -> (usize, usize) {
...
fn try_move_two(
    start: (usize, usize),
    dir: &Move,
    contents: &mut nalgebra::DMatrix<ItemTwo>,
) -> (usize, usize) {
    let new_position = movement_target(start, &dir);
    match contents[new_position] {
        ItemTwo::Wall => {
...
fn can_move(
    empty_pos_l: (usize, usize),
    empty_pos_r: (usize, usize),
    dir: &Move,
    contents: &nalgebra::DMatrix<ItemTwo>,
) -> bool {
    ...
    if contents[next_position_l] == ItemTwo::ObjectLeft
        && contents[next_position_r] == ItemTwo::ObjectRight
    {
    ...
    if contents[next_position_l] == ItemTwo::ObjectRight && *dir != Move::Right {
        can_move_both = can_move_both
            && can_move(
                (next_position_l.0, next_position_l.1 - 1),
                next_position_l,
                dir,
                contents,
            );
    ...
    if contents[next_position_r] == ItemTwo::ObjectLeft && *dir != Move::Left {
        can_move_both = can_move_both
            && can_move(
                next_position_r,
                (next_position_r.0, next_position_r.1 + 1),
                dir,
                contents,
            );
    }
...
fn move_object(
    position_l: (usize, usize),
    position_r: (usize, usize),
    dir: &Move,
    contents: &mut nalgebra::DMatrix<ItemTwo>,
) {
