...
fn get_xy(input: &str) -> (i64, i64) {
    ...
    (i64::from_str_radix(parts[0],10).unwrap(), i64::from_str_radix(parts[1], 10).unwrap())
}
...
fn move_robot(robot: &Robot, num_moves: i64) -> (i64, i64) {
    ...
    (position.0 + robot.speed.0 * num_moves, position.1 + robot.speed.1 * num_moves)
}
...
fn show_iteration(robots: &Vec<Robot>, time: u32, area: (usize, usize)) -> nalgebra::Matrix<char, nalgebra::Dyn, nalgebra::Dyn, nalgebra::VecStorage<char, nalgebra::Dyn, nalgebra::Dyn>>  {
    ...
    for robot in robots {
        ...
        let position_matrix: (usize, usize) = (position.1.try_into().unwrap(), position.0.try_into().unwrap());
...
fn find_area(matrix: &nalgebra::Matrix<char, nalgebra::Dyn, nalgebra::Dyn, nalgebra::VecStorage<char, nalgebra::Dyn, nalgebra::Dyn>>) -> bool {
    for y in 0..matrix.nrows() - 4 {
        for x in 0..matrix.ncols() - 1 {
            if matrix[(y, x)] == '#' && matrix[(y + 1, x)] == '#' && matrix[(y+2, x)] == '#' && matrix[(y+3,x)] == '#' && matrix[(y+4, x)] == '#' 
                && matrix[(y, x+1)] == '#' && matrix[(y + 1, x+1)] == '#' && matrix[(y+2, x+1)] == '#' && matrix[(y+3,x+1)] == '#' && matrix[(y+4, x+1)] == '#'{
                return true;
            }
        }
    }
